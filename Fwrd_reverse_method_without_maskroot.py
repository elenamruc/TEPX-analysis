...
# -*- coding: utf-8 -*-
"""
Fwd-Reverse SCurve analysis

Uso:
  python fwd_reverse.py  fwd.root  rev.root  [out_dir]
...
# ─────────────── análisis por chip ───────────────
def analyse_chip(chip, mod, fwd, rev, root_out, out_dir):
    ...
    if None in (h_thr_f,h_thr_r,h_noi_f,h_noi_r):
    ...
    draw_summary_manual_mask(chip, mod, dThr, dNoi, out_dir, root_out, chip_dir, h_thr_f)
    
def draw_summary_manual_mask(chip, mod, dThr, dNoi, out_dir, root_out, chip_dir, h_thr_f):
    nx, ny = h_thr_f.GetNbinsX(), h_thr_f.GetNbinsY()  # número de píxeles por eje, puedes ajustarlo si es variable
    print(nx)
    print(ny)
    h_miss = ROOT.TH2I("h_miss", "", nx, 0, nx, ny, 0, ny)
    h_prob = ROOT.TH2I("h_prob", "", nx, 0, nx, ny, 0, ny)

    # Solo define h_mask si es necesario
    if chip == 15 and mod == 102:
    ...
    for ix in range(1, nx + 1):
        for iy in range(1, ny + 1):
            masked = False
            if chip == 15 and mod == 102 and ix > nx - 8:
                masked = True
            
            if masked:
                h_mask.SetBinContent(ix, iy, 1)
                #print(f"✓ MASKED: chip={chip}, mod={mod}, ix={ix}, iy={iy}")
                continue
    ...
    leg.AddEntry(h_prob, f"Problematic ({len(coords['prob'])})", "f")
    if h_mask:
        leg.AddEntry(h_mask, f"Masked (2688)" if chip == 15 else "Masked", "f")
    leg.Draw()
...
def main():
    ...
    fwd_path, rev_path = sys.argv[1:3]
    mod = int(sys.argv[3])
    out_dir = sys.argv[4] if len(sys.argv) > 4 else "FwdReverse_analysis"
    ...
    for chip in (15,14,13,12):
        analyse_chip(chip,mod,fwd,rev,root_out,out_dir)

    root_out.Close(); fwd.Close(); rev.Close()
